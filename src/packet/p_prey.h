#ifndef SRC_PACKET_P_PREY_H_
#define SRC_PACKET_P_PREY_H_
...
struct packet_update_prey : public PacketBase {
  ...
  uint8_t direction;  // int8    value - 48 -> direction (0: not turning; 1:
                      // turning counter-clockwise; 2: turning clockwise)
  uint24_t angle;     // int24    value * 2*PI / 16777215 -> current angle
  uint24_t
      wangle;   // int24    value * 2*PI / 16777215 -> wanted angle (angle the
                // prey turns towards)
  ...
  // TODO(john.koepi): size_t get_size() const noexcept { return 3 + 6 + ...; }
};
...
struct packet_remove_prey : public PacketBase {
  ...
  // TODO(john.koepi): size_t get_size() const noexcept { return ...; }
};

#endif  // SRC_PACKET_P_PREY_H_
